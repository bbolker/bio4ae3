...
- **gene-for-gene**: like inverse matching, but there is a "universal infector" that doesn't match anything (i.e. infects everything) (Plants: R genes, Avr genes). [Wikipedia](https://en.wikipedia.org/wiki/Gene-for-gene_relationship):
...
(breaking up co-adapted gene complexes: @goldbergIncreased2005).
...
helps the RQ work.
(Asexual lineages are very common in snails.)

Primary theories for the variation in frequency of sexual snails among
and within lakes:

- **Resistance tradeoffs:** genetic tradeoff between competitive ability and
resistance to parasites
-   **Reproductive assurance:** asexuals ensure reproduction and avoid costs
of mating (assuming sexuals have some other advantage)
...
```{r lively_tab, echo=FALSE, warn=FALSE, message= FALSE, results = "asis"}
library(tidyverse)
...
(kable(tt)
    ...
    |> add_header_above(c(" " = 2, "resistance\ntradeoffs" = 2, "reproductive\nassurance" = 2, "lottery" = 2,
                          "tangled bank" = 1, "Red Queen" = 3))
)
```

<!-- https://stackoverflow.com/questions/44490209/how-to-change-font-size-of-table-in-rmarkdown-latex-and-pdf -->
<!--https://stackoverflow.com/questions/27626461/hiding-nas-when-printing-a-dataframe-in-knitr -->
...
off from the sexual population and how, etc. etc.. (Importance of **epistasis**: [@metzger_red_2016])
